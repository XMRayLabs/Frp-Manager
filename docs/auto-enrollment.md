...
在客户端或服务端列表的操作菜单选择“修改 ID”，保存新的实际节点 ID。仅节点所有者或同租户管理员可操作；内置服务端、临时客户端和内部子配置不支持单独改名。

改名会同步迁移隧道、子配置、流量统计和共享权限的关联。正在使用的连接保留，旧版内核无需重新安装。已保存的设备身份和旧安装命令通过原 ID 兼容到新 ID；原 ID 永久保留，不能用于新节点。节点删除后，旧身份也失效。已有隧道名称保持不变，之后创建的隧道使用新 ID 生成名称。
