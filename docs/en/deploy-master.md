...
You have three deployment options鈥攃hoose one.
...
**Note鈿狅笍:** First deploy successfully using the regular method! Then adjust these two variables!!!!
...
docker run -d -p 9000:9000 \ # API console port
  ...
  --restart=unless-stopped \
  -v /opt/frp-manager:/data \ # Data storage location
...
services:
  ...
  frpp-master:
    image: sakurame1/frp-manager:1.0.0 # Change to the version you want to use
    environment:
