...
**Note 鈿狅笍:** The startup commands for `client` and `server` may change as the project evolves. Although we strive for backward compatibility, the commands generated by the Master鈥檚 web UI should be treated as authoritative.
...
> If you only have one public-facing server to manage, you can use the Master鈥檚 built-in `default` server without deploying a separate Server. Remember to configure the `default` server after starting the Master.

## Deploying on Linux

### 1. Preparation

Open the Master鈥檚 web UI and log in. If you don鈥檛 have an account, register鈥攜our first user will be the administrator.
...
If you are using a reverse-proxy with TLS, proxy external port 443 to the Master鈥檚 port 9000 (API) over HTTP upstream, and adjust the command accordingly:
