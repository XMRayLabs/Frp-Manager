...
In addition, we also need to consider the compatibility with the original version. The client/server of frp-manager must be able to connect to the official frpc/frps service. In this way, both configuration file and non-configuration file modes can work perfectly.
...
|   |-- client          # Client logic (here referring to the frp-manager client)
|   |-- master          # frp-manager control plane, responsible for handling frontend requests, and using rpc to manage frp-manager's server and client
