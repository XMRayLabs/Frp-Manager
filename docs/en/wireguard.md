...
- Dev notes: [WireGuard multi-hop smart networking development log](https://github.com/Sakurame1/frp-manager), includes a funny demo you can try.

> The networking feature is currently in beta. Please report problems through GitHub Issues.
>
> Networking currently supports Linux hosts only.

`frp-manager` bundles `wireguard-go` to provide networking. It supports WireGuard over UDP/WebSocket multi-hop networking, custom routes and topology, and calculates the shortest path using latency and bandwidth so it works well even in complex network environments.
...
3. At minimum, `frp-manager master` and `frp-manager client` are installed.
4. `frp-manager client` must be installed with root privileges.
...
This lets the right client communicate with the left client through `frp-manager` networking.
...
- **CIDR**: The network CIDR, e.g., `10.10.0.0/24`. This example covers `10.10.0.0`鈥揱10.10.0.255`.
...
- **Local Address**: The device鈥檚 virtual IP, e.g., `10.10.0.2`. Must be within the network range. The default value auto-assigns an available IP.
...
You can drag from a node鈥檚 right black dot to another node鈥檚 left black dot to create a connection. When creating manual connections, avoid selecting endpoints to prevent issues when endpoints change.
