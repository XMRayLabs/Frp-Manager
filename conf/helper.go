...
func JWTSecret(cfg Config) string {
	return utils.SHA256(fmt.Sprintf("%s:%d:%s", cfg.Master.APIHost, cfg.Master.APIPort, cfg.App.GlobalSecret))
}
...
func GetCertTemplate(cfg Config) *x509.Certificate {
	...
	return &x509.Certificate{
		...
		Subject: pkix.Name{
			...
			Country:            []string{"CN"},
			Organization:       []string{"frp-manager"},
			OrganizationalUnit: []string{"frp-manager"},
		},
