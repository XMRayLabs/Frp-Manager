...
func spawnWorker(exePath string, planPath string) error {
	...
	workerPath, err := prepareWorkerExecutable(exePath, planPath)
	if err != nil {
		return err
	}
	return spawnDetached(workerPath, []string{"__upgrade-worker", "--plan", planPath})
}

func spawnDetached(exePath string, args []string) error {
	if len(exePath) == 0 {
		return fmt.Errorf("exePath is empty")
	}
