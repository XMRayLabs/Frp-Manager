...
import { getNetworkTopology, getWireGuardRuntimes, listWireGuards } from '@/api/wg'
import { GetNetworkTopologyRequest, GetWireGuardRuntimeInfosRequest, ListWireGuardsRequest } from '@/lib/pb/api_wg'
...
export function useNetworkTopology(networkID?: number, spf?: boolean) {
  ...
  const { data: runtimeData, isFetching: isRuntimeFetching, refetch: refetchRuntime } = useQuery({
    queryKey: ['listWireGuardRuntimes', networkID],
    queryFn: async () => {
      if (!networkID) return undefined
      return await getWireGuardRuntimes(GetWireGuardRuntimeInfosRequest.create({ networkId: networkID }))
    },
    enabled: !!networkID,
    staleTime: 8000,
  ...
  const topology: TopologyData = useMemo(() => {
    ...
    sortedIds.forEach((idStr) => {
      ...
      nodes.push({
        ...
        data: {
          label,
          config,
          runtime: runtimeData?.runtimeInfos[id],
        },
    ...
    return { nodes, edges }
  }, [topologyData?.adjs, wgList?.wireguardConfigs, runtimeData?.runtimeInfos])

  const isFetching = isTopologyFetching || isWgListFetching || isRuntimeFetching
