...
import { getClientStatus } from '@/api/status-batcher'
...
const WGNodeComponent: React.FC<NodeComponentProps> = ({
  ...
  const { data: clientStatus } = useQuery({
    queryKey: ['clientStatus', ClientType.FRPC, clientId],
    queryFn: async () => {
      if (!clientId) return undefined
      return await getClientStatus(ClientType.FRPC, clientId)
    },
    enabled: !!clientId,
    staleTime: 8000,
