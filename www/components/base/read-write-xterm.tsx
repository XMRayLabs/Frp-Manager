...
const TerminalComponent = ({ isLoading, clientStatus, reset, setStatus }: TerminalComponentProps) => {
  ...
  const clientPlatform = clientStatus?.version?.platform
  ...
  useEffect(() => {
    ...
    terminalWebsocketUrl(clientID, terminal.rows, terminal.cols)
      .then((url) => {
        if (disposed) {
        ...
        websocket.addEventListener('open', () => {
          ...
          terminal.options = {
            disableStdin: false,
            windowsMode: clientPlatform?.includes('windows'),
          }
  ...
  }, [clientID, clientPlatform, terminal, isLoading, setStatus])
