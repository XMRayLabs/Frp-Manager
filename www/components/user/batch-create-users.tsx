...
export function BatchCreateUsersDialog({ onCreated }: { onCreated: () => Promise<void> }) {
    ...
    <Dialog open={open} onOpenChange={(value) => { if (!busy) setOpen(value) }}>
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        ...
        <form className="grid gap-4" onSubmit={async (event) => {
          ...
          setBusy(true)
          try {
            const created = await batchCreateUsers({ username: username.trim(), email: email.trim(), count })
          ...
          <Button type="submit" disabled={!valid || busy}>{busy ? '正在创建…' : `创建 ${count} 个用户`}</Button>
        </form>
      </DialogContent>
    </Dialog>
  )
