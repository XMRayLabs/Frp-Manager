...
export function WorkerStatus({ workerId, clients = [], compact = false }: WorkerStatusProps) {
  ...
  const ingressStatuses = useQuery({
    ...
    queryFn: async () => {
      const response = await getProxyStatuses({
        proxyIds: ingresses.map((ingress) => Number(ingress.id ?? 0)).filter((id) => id > 0),
      })
      return Object.fromEntries(
        response.proxyStatuses.map((result) => [
          String(result.proxyId ?? 0),
          result.workingStatus?.status || 'unknown',
        ]),
      )
    },
