...
import { getProxyStatus } from '@/api/proxy-status-batcher'
...
function ProxyStatusBadge({
  ...
  const { data } = useQuery({
    queryKey: ['getProxyStatus', proxyId, refetchTrigger],
    queryFn: () => getProxyStatus(proxyId || 0),
    enabled: !!proxyId && !!clientId && !!serverId && !!proxyName,
    staleTime: 8000,
