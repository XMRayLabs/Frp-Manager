...
import {
  ...
  GetProxyStatusesRequest,
  GetProxyStatusesResponse,
...
export const getProxyStatuses = async (req: GetProxyStatusesRequest) => {
  const res = await http.post(API_PATH + '/proxy/get_statuses', GetProxyStatusesRequest.toJson(req))
  return GetProxyStatusesResponse.fromJson((res.data as BaseResponse).body)
}
