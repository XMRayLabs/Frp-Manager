...
func (w *WSConn) readLoop(ctx *app.Context) {
	...
	// 闄愬埗鍗曟潯娑堟伅澶у皬锛岄伩鍏?gorilla/websocket.ReadMessage/io.ReadAll 瑙﹀彂瓒呭ぇ鍒嗛厤
	...
	for {
		...
		for offset+2 <= len(data) {
			...
			case incomingChan <- pkt:
				// 鎴愬姛鍙戦€?
			case <-done:
				// context 宸插彇娑堬紝褰掕繕骞堕€€鍑?
				w.wsBind.packetPool.Put(pkt)
				return
			default:
				// channel 婊′簡锛屼涪寮冨寘骞跺綊杩樺埌瀵硅薄姹?
				w.wsBind.packetPool.Put(pkt)
			}
