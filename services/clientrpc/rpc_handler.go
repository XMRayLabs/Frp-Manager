...
func registClientToMaster(appInstance app.Application, recvStream pb.Master_ServerSendClient, event pb.Event, clientID, clientSecret string) error {
	ctx := context.Background()
	logger.Logger(ctx).Infof("start to regist client to master")
	version, _ := proto.Marshal(conf.GetVersion().ToProto())
	for {
		...
		if err != nil {
			return fmt.Errorf("receive client registration response: %w", err)
		}

		if resp.GetEvent() == event {
			logger.Logger(ctx).Infof("client registration succeeded, clientID: %s", resp.GetClientId())
			return nil
		}
	}
}
	...
	for {
		...
		default:
			...
			go func() {
				...
				sendMu.Lock()
				err := recvStream.Send(msg)
				sendMu.Unlock()
				if err != nil {
					logger.Logger(c).WithError(err).Errorf("cannot send client response")
					return
				}
...
func startClientRpcHandler(appInstance app.Application, client app.MasterClient, done chan bool, clientID, clientSecret string, event pb.Event,
	...
	for {
		...
		default:
			...
			if err := registClientToMaster(appInstance, recvStream, event, clientID, clientSecret); err != nil {
				logger.Logger(c).WithError(err).Warn("client registration failed; retrying in 3 seconds")
